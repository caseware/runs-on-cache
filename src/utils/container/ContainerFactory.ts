...
type Instances = {
    [K in keyof typeof SUPPORTED_CLASSES]: InstanceType<typeof SUPPORTED_CLASSES[K]>;
};
...
export class ContainerFactory {
    static getCacheContainer(
        ...
        const foundInstanceKey = (Object.keys(instances) as (keyof typeof instances)[]).find(
            key => instances[key].isSupportedMethod(customCompression)
        );
